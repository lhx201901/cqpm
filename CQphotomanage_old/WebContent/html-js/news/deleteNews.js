...
function load_user(){
	var  obj = getParam();
	$.ajax({
		url : "/photo/photoDelNotice/findPageInfo", // 请求的url地址
		...
		success : function(data) {
			...
			var url = "/photo/photoDelNotice/findPageInfo";
...
function loadOperater(data){
	var html ="";
	html += '<a  onclick="picDetail(\''+data.picXh+'\')">查看</a>';
	if(data.operationStatu==0){
		html += '<a class="ml10" onclick="asyncPhoto(\''+data.id+'\')">同步</a>';
	}else{
...
function asyncPhoto(id){
	...
	table += '</td></tr>';
	table += '</table>';
	var html='<a class="btn btn_ok" onclick="sureasync(1)">同步删除</a>'
		+'<a class="btn ml10" onclick="sureasync(0)">不同步</a>';
	AlertBox.audit(table, "删除提示",html);		
	$("#dataId").val(id);
}
function sureasync(async){
	var obj={};
	obj.dataId=$("#dataId").val();
	obj.opreateInfo=$("#remarks").val();
	obj.async=async;
	AlertBox.alert("同步操作中请稍等！", "删除提示");
	console.log(obj);
	$.ajax({
		url : "/photo/photoDelNotice/asyncPhotoDelNoticeInfo",
		dataType : "json",
		async : true,
		data :obj
		,
		...
		success : function(data) {
			if(data){
				AlertBox.alert("同步成功!","消息提示");
				AlertBox.onHide(function(){
					searchData();
					loadMessgeNum();
				});
			}else{
				AlertBox.alert("同步失败！","消息提示");
			}
...
function picDetail(pic_xh){
	var url = "../../html/pics/picDetail.html?uuid="+pic_xh+"&type=d_photo_pic";
	url = encodeURI(url);
	parent.createPage("详情",url , true, "picsDetail"+pic_xh);
}
function auditInfo(id){
	$.ajax({
		url : "/photo/photoDelNotice/findById",
		dataType : "json",
		async : true,
		data :{id : id},
		type : "post",
		success : function(data) {
			var table = "";
			table += '<table style ="margin: 0 auto;" >';
			table += '<tr><td>审核人:</td><td style="text-align:left;"><span id="auditUser" ></span></td></tr>';
			table += '<tr><td>审核时间:</td><td style="text-align:left;"><span id="auditTimes"></span></td></tr>';
			table += '<tr><td>备注:</td><td style="text-align:left;">';
			table += '<textarea id="remarks" rows="3" cols="20" style="text-align:left;"></textarea>';
			table += '</td></tr><br>';
			table += '<tr><td colspan="2" align="center"><input type="hidden" id="dataId" >';
			table += '</td></tr>';
			table += '</table>';
			AlertBox.audit(table, "审核信息","");		
			$('#auditUser').html("&nbsp;&nbsp;&nbsp;"+data.operationUserName);
			$('#auditTimes').html("&nbsp;&nbsp;&nbsp;"+data.operationTime);
			$('#remarks').val(data.operationReason);
		},
		error : function() {
			AlertBox.alert("系统错误，请稍后重试！");
		}
...
function sureDelete(id){
	$.ajax({
		url : "/photo/photoDelNotice/deleteByIds",
		dataType : "json",
		async : true,
		data : {
			ids : id
