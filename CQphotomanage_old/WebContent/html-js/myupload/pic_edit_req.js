 ...
 * 查询图片编辑请求
 */
function findByEditXh(){
	$.ajax({
		url : "/photo/photoEdit/findByPicXhAndNotEdit", 
		dataType : "json", 
		async : true,
		data : {
			picXh:XH
...
function getNweData(){
	...
	if(_OBJ.sendTime!=undefined && _OBJ.sendTime!= null && _OBJ.sendTime!= ""){
		obj.sendTime = new Date(_OBJ.sendTime);
	}else{
		delete obj.sendTime;
	}
	delete obj.editTime;
	obj.sendRemark=$("#sendRemark").val();
	return obj;
}